...
3. Yes we can identify. For exapmle: Component 1 (US and Canada treasury and bonds futures), Component 2 (Currency futures), Component 3(European stock expange indexes), Component 4 (American stock exchange indexes), Component 5 (European interest rates), Component 6 (European interest rates futures), Component 7 (oil futures), Component 8 (metals futures), Component 9 (agro futures), Component 10 (asian indexes futures).

4(a). Principal components and scree plot for financial crisis years:
...
4(b). Principal components and scree plot for financial pre-crisis years:
...
5. During the **bear years** variance in Component 1 (US and Canada treasury and bonds futures) explains almost 30% of all the variance in futures returns and there are 20 Components with Eigenvalue >1.  of all variance explained by Components 1-4.  During the **bull years** variance in Component 1 (US and Canada treasury and bonds futures) explains almost 18% of all the variance in futures returns and there are 28 Components with Eigenvalue >1. This may imply that during crisis periods investors prefer to sell less "popular" or risky futures and buy safer instruments like those included in Component 1.
...
1. Only in broad lines (I am still strugling but it becomes better)

2. If I make MAfreq shorter and apply moving average function to a longer period (40 days), results of my trading strategy look like this (more successful for some years, less for another):


```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(10,250,by=40)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
