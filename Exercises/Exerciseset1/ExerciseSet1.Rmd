...
I downloaded basic R package with called _jrvFinance_ that implements the basic financial analysis functions similar to (but not identical to) what is available in most spreadsheet software.
...
1. It is done in dataSet1.R file in lines 11-18
2. The cumulative returns of the Apple stock during this period is `r round(100*sum(StockReturns[,"AAPL"]),1)`%. The average daily returns of the Apple stock during this period is `r round(100*mean(StockReturns[,"AAPL"]),3)`%. The standard deviation of the daily returns of the Apple stock during this period is `r round(100*sd(StockReturns[,"AAPL"]),3)`%;
<br>
3. The cumulative returns of the Yahoo! stock during this period is `r round(100*sum(StockReturns[,"YHOO"]),1)`%. The average daily returns of the Yahoo! stock during this period is `r round(100*mean(StockReturns[,"YHOO"]),3)`%. The standard deviation of the daily returns of the Yahoo! stock during this period is `r round(100*sd(StockReturns[,"YHOO"]),3)`%;
...
2. Here is a plot of returns for portfolio with S&P and Apple (excluding Yahoo that was previously included in StockReturns): 
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
transposedData = t(StockReturns)
transposedData1 = transposedData[c(1:2),] # I create average portfolio fo just S&P and Apple excluding Yahoo!
portfolio = apply(transposedData1,2,mean)
names(portfolio) <- colnames(transposedData1)
pnl_plot(portfolio)
```
...
**Your Answers here:**
1. I need to change startDate = "2005-01-01" to "2001-01-01" in dataSet.R
